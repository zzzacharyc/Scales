...
* Your scale needs a shape that is either complex (i.e. uses [`bezier()`](https://processing.org/reference/bezier_.html), `vertex()`, `curveVertex()` or similiar) or composite (made from combining multiple simple shapes like `rect()` and `ellipse()`). The shape should not be so simple that it could be drawn with a single Processing graphics function.
